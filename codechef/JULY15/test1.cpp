...
ll t[M];
ll test(int type,int x,int y,int v){
	switch(type){
		case 1:
			for(int i=x;i<=y;++i)
				t[i] = (t[i]+v)%MOD;
...
while(1){	
	...
	int n = rand()%100+1;cout << n <<' ';
	int m = rand()%100+1;cout << m <<endl;
	{
		FRE(i,n)
			cout << (t[i] = tree.ori[i] = rand()) << ' ';
		cout << endl;
		...
		while(m--){
			...
			if(x>y)
				swap(x,y);
			if(type!=4) v = rand();
			cout <<type <<' '<<x <<' '<< y <<' ' << v << "xxxxxxxx"<<endl;
			if(type==4){
				if(test(type,x,y,v) !=  tree.ask(x,y) ){
					cout<< ("OH~NO~---------------------------\n") ;
					FRE(i,n)
						cout << t[i] << ' ';
					cout << "==========="<<endl;
					tree.postorder(tree.root);
					cout << "ans:" << tree.ask(x,y) << " " <<test(type,x,y,v) << endl;
					f.close();
					return 0;
				}
